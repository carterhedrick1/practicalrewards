...
- Calculation values carry units that are checked mechanically, so write each value in exactly one of these forms: "$300" (dollars), "50,000 points" or "50k points" or "25000 miles" (points), "1.5cpp" (cents per point/mile), "3x" (points per dollar), "6%" (percent), or a bare number like "4" (unitless count). Units must compose: points × cpp = dollars; dollars × points-per-dollar = points; dollars ÷ points = cpp; dollars ÷ cpp = points; points ÷ dollars = points-per-dollar; same unit ÷ same unit = ratio (write the result as a percent or bare number); percent and bare counts act as plain multipliers. Add and subtract only accept matching units. Never mix forms that do not compose — to turn points into dollars, use a cpp value, not a dollar figure per point. The result's unit must follow from the inputs: "50,000 points" × "1.5cpp" has result "$750", and "$395" ÷ "$500" has result "79%".
